"""
Scrape, embed, and index all HSK levels (HSK1–HSK6) for Mandarin Chinese.

Usage (run from project root):
    python scripts/index_hsk.py                    # grammar + vocabulary, all levels
    python scripts/index_hsk.py --levels HSK3 HSK4  # specific levels only
    python scripts/index_hsk.py --type grammar      # grammar only
    python scripts/index_hsk.py --type vocabulary   # vocabulary only
    python scripts/index_hsk.py --dry-run           # scrape only, no embedding
"""
import argparse
import sys
import time
...
from config.languages import LEVELS_HSK_CLASSIC
...
def _get_or_create_newsletter() -> dict:
    """Return the first Mandarin Chinese / HSK newsletter, creating one if needed."""
    newsletters = get_newsletters()
    for nl in newsletters:
        if nl["language"] == LANGUAGE and nl["exam"] == EXAM:
            return nl
    nl_id = insert_newsletter(name="Mandarin Chinese HSK", language=LANGUAGE, exam=EXAM)
...
def _index_level(nl: dict, level: str, content_type: str,
                 scraper: MandarinHSKScraper, collection,
                 dry_run: bool) -> dict:
    """Scrape, store, and embed one level+content_type. Returns a stats dict."""
    ...
    # Attach deterministic vector IDs
    for chunk in raw_chunks:
        chunk["chroma_doc_id"] = make_chunk_id(chunk["source_url"], chunk["chunk_index"])

    # Group by source URL, write scrape sessions + chunks to SQLite
    url_groups: dict = {}
    for chunk in raw_chunks:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Index HSK content into vector store")
    parser.add_argument(
        "--levels", nargs="+", default=LEVELS_HSK_CLASSIC,
        metavar="LEVEL",
        help="HSK levels to index (default: all HSK1–HSK6)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Scrape and write to SQLite but skip OpenAI embedding",
    )
    ...
    for i, level in enumerate(args.levels, 1):
        ...
        for ct in content_types:
            ...
            if stats["error"]:
                ...
                print(f"{stats['scraped']} chunks scraped (dry-run, not embedded)  [{elapsed:.1f}s]")
            else:
                print(f"{stats['scraped']} chunks scraped, {stats['embedded']} embedded  [{elapsed:.1f}s]")

        # Brief pause between levels to be polite to the servers
        if i < total_levels:
            time.sleep(1.5)
