"""
Scrape, embed, and index all JLPT levels (N5–N1) for Japanese.

Usage (run from project root):
    python scripts/index_jlpt.py                    # grammar + vocabulary, all levels
    python scripts/index_jlpt.py --levels N5 N4     # specific levels only
    python scripts/index_jlpt.py --type grammar     # grammar only
    python scripts/index_jlpt.py --type vocabulary  # vocabulary only
    python scripts/index_jlpt.py --dry-run          # scrape only, no embedding
"""
import argparse
import sys
import time
...
from config.languages import LEVELS_JLPT
...
def _get_or_create_newsletter() -> dict:
    ...
    for nl in newsletters:
        if nl["language"] == LANGUAGE and nl["exam"] == EXAM:
            return nl
    nl_id = insert_newsletter(name="Japanese JLPT", language=LANGUAGE, exam=EXAM)
...
def _index_level(nl: dict, level: str, content_type: str,
                 scraper: JLPTScraper, collection,
                 dry_run: bool) -> dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Index JLPT content into vector store")
    parser.add_argument(
        "--levels", nargs="+", default=LEVELS_JLPT,
        metavar="LEVEL",
        help="JLPT levels to index (default: all N5–N1)",
    )
    parser.add_argument(
